...
- 一只鸽同一时刻只能有一张进行中（待验收/冻结）的调运单；观察期（观察中 **或** 解除申请待复核，直到管理员作出结论）禁止发起与任何归属流转。
- 发起后鸽只置为「在途」，验收后落地目标棚；拒收退回来源棚；验收后可再退回（须填原因）。
- 状态机：`pending → accepted → returned`；`pending → rejected`；疫病期间 `pending ⇄ frozen`。非法跳转一律 422。
- **观察期内验收、拒收、退回一律 423 拒绝**（在途单此时已冻结，其验收/拒收先得 422）；失败整体回滚，单据状态、版本号、时间线与鸽只归属均不变。只有复核解除观察后才恢复流转。
